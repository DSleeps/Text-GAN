...
class Decoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, output_size, device):
        ...
        self.output_model = nn.Sequential(nn.Linear(hidden_size, output_size),
                                          nn.LogSoftmax())
    ...
    def forward(self, inputs, lengths, final_hidden, hiddens, max_output_len, max_sequence_len):
        ...
        hidden_outs = torch.zeros(batch_size, max_output_len, self.hidden_size).to(self.device)
        ...
        for i in range(max_output_len):
            ...
            outputs[:,i] = self.output_model(output[:,0,:])
            hidden_outs[:,i] = output[:,0,:]

        return outputs, hidden_outs
