...
from TanGAN import Generator, Discriminator
...
for i in range(iteration_count): 
    ...
    for d in range(d_times):
        ...
        rand_ends += torch.arange(int(batch_size/2)).to(device) * max_output_len
        mask = torch.arange(int(batch_size/2)*max_output_len).view(int(batch_size/2),max_output_len).to(device)
        mask = mask < rand_ends.view(int(batch_size/2),1)
    ...
    for g in range(g_times):
        ...
        rand_ends += torch.arange(batch_size).to(device) * max_output_len
        mask = torch.arange(batch_size*max_output_len).view(batch_size,max_output_len).to(device)
        mask = mask < rand_ends.view(batch_size,1)
